...
use agent_evolution::{
    ...
    open_ended_evolution::{OpenEndedEvolution, NoveltyDetectionOutput},
    genetics::Genome,
};
use anyhow::Result;
...
impl AIAgent {
    ...
    fn process_learning_cycle(&mut self, environment_state: &DVector<f64>, cycle: u64) -> Result<AgentOutput> {
        ...
        self.hypernetwork.adapt_from_feedback(&task_embedding, outcome)?;
    ...
    fn generate_task_embedding(&self, environment_state: &DVector<f64>) -> Result<DVector<f64>> {
        let mut embedding = Vec::new();
        
        // Add environment features
        embedding.extend(environment_state.iter().take(5));
        
        // Add agent state features
        embedding.push(self.performance_history.last().unwrap_or(&0.0));
        embedding.push(&(self.discovery_count as f64));
        embedding.push(&(self.generation as f64));
        
        // Pad to fixed size
        while embedding.len() < 10 {
            embedding.push(&0.0);
        }
        
        Ok(DVector::from_vec(embedding.into_iter().cloned().collect()))
    }

    fn adapt_neural_architecture(&mut self, architecture: &agent_evolution::hypernetwork::GeneratedArchitecture) -> Result<()> {
        self.neural_core.learning_rate = architecture.learning_rate;
        self.neural_core.exploration_rate = architecture.exploration_rate;
        Ok(())
    }
    ...
    fn update_meta_learning(&mut self, outcome: f64, cycle: u64) -> Result<()> {
        let input = agent_evolution::meta_learning::MetaLearningInput {
            timestamp: cycle as f64,
            performance: outcome,
            learning_rate: self.neural_core.learning_rate,
            exploration_rate: self.neural_core.exploration_rate,
            adaptation_rate: self.hypernetwork.adaptation_rate,
            novelty_score: 0.5,
            complexity: 0.3,
        };
        
        self.meta_learner.update(input)?;
        
        if let Some(learning_config) = self.meta_learner.param_configs.get(&agent_evolution::meta_learning::MetaParameter::LearningRate) {
            self.neural_core.learning_rate = learning_config.current_value;
        }
        
        if let Some(exploration_config) = self.meta_learner.param_configs.get(&agent_evolution::meta_learning::MetaParameter::ExplorationRate) {
            self.neural_core.exploration_rate = exploration_config.current_value;
        }
    ...
    fn get_statistics(&self) -> AgentStatistics {
        ...
        AgentStatistics {
            ...
            meta_learning_stats: self.meta_learner.get_statistics(),
...
struct AgentStatistics {
    ...
    meta_learning_stats: agent_evolution::meta_learning::MetaLearningStatistics,
...
fn report_agent_statistics(stats: &AgentStatistics) {
    ...
    println!("   Meta-Learning Adaptations: {}", stats.meta_learning_stats.total_adaptations);
