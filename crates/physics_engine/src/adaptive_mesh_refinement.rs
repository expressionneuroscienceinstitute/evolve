    ...
    fn refine_cell(&mut self, cell_id: usize, current_time: f64) -> Result<()> {
        ...
        let (parent_level, parent_pos, parent_size, boundary_conditions, refinement_criterion) = {
            if let Some(parent_cell) = self.cells.get(cell_id) {
                ...
                (
                    parent_cell.level,
                    parent_cell.position,
                    parent_cell.size,
                    parent_cell.boundary_conditions.clone(),
                    parent_cell.refinement_criterion,
                )
            } else {
        ...
        // Phase 5: Record refinement event
        self.refinement_history.push(RefinementEvent {
            timestamp: current_time,
            cell_id,
            ...
            trigger_value: refinement_criterion,
        });
